...
def test(model, device, test_loader, output_file):
    ...
    for (input_tensor1, input_tensor2, input_tensor3, slideID, class_label, domain_label, pos_list) in test_loader:
...
def test_model(train_slide, test_slide):
    ...
    makedir('test_att')
    result = f'test_att/test_{mag1}_{mag2}_{mag3}_train-{train_slide}_DArate-{DArate}_ms.csv'
    ...
    model_params = f'../../../../nvme/hashimoto.n/Output/model_params/{mag1}_{mag2}_{mag3}_train-{train_slide}_DArate-{DArate}_epoch-{epoch_m}_ms.pth'
    feature_extractor_mag1 = feature_extractor()
    feature_extractor_mag2 = feature_extractor()
    feature_extractor_mag3 = feature_extractor()
    class_predictor = class_predictor()
    #domain_predictor = domain_predictor(domain_num)
    model = MSDAMIL3(feature_extractor_mag1, feature_extractor_mag2, feature_extractor_mag3, class_predictor)# DAMIL構築
    ...
    hori_test = HoriDataset.HoriDataset_multi3(
        train=False,
        transform=transform,
        pyvips=False,
