...
def train(model, rank, loss_fn, optimizer, train_loader):
    ...
    correct_num = 0
    for (input_tensor1, input_tensor2, input_tensor3, slideID, class_label, domain_label) in train_loader:
        # MILとバッチ学習のギャップを吸収
        input_tensor1 = input_tensor1.to(rank, non_blocking=True)
        input_tensor2 = input_tensor2.to(rank, non_blocking=True)
        input_tensor3 = input_tensor3.to(rank, non_blocking=True)
        class_label = class_label.to(rank, non_blocking=True)
        for bag_num in range(input_tensor1.shape[0]):
            optimizer.zero_grad() #勾配初期化
            class_prob, class_hat, A = model(input_tensor1[bag_num], input_tensor2[bag_num], input_tensor3[bag_num])
            # 各loss計算
...
def valid(model, rank, loss_fn, test_loader):
    ...
    test_class_loss = 0.0
    correct_num = 0
    for (input_tensor1, input_tensor2, input_tensor3, slideID, class_label, domain_label) in test_loader:
        # MILとバッチ学習のギャップを吸収
        input_tensor1 = input_tensor1.to(rank, non_blocking=True)
        input_tensor2 = input_tensor2.to(rank, non_blocking=True)
        input_tensor3 = input_tensor3.to(rank, non_blocking=True)
...
def train_model(rank, world_size, train_slide, valid_slide):
    ...
    mag2 = '20x' # ('5x' or '10x' or '20x')
    mag3 = '5x' # ('5x' or '10x' or '20x')
    EPOCHS = 10
    DA = 'False'
    if DA == 'True':
        DArate = 0.01
    ...
    if rank < 9:
        makedir('train_log')
        log = f'train_log/log_{mag1}_{mag2}_{mag3}_train-{train_slide}_DArate-{DArate}_ms.csv'
        makedir('valid_result')
        valid_result = f'valid_result/{mag1}_{mag2}_{mag3}_train-{train_slide}_valid-{valid_slide}_DArate-{DArate}_ms.csv'
        ...
        log3 = f'train_log/log_{mag3}_train-{train_slide}_DArate-{DArate}_opt.csv'
    ...
    # model読み込み
    from model import feature_extractor, class_predictor, domain_predictor, DAMIL, MSDAMIL, MSDAMIL3
    # 各ブロック宣言
    epoch1 = select_epoch(log1)
    epoch2 = select_epoch(log2)
    epoch3 = select_epoch(log3)
    if rank == 0:
        print(epoch1)
        print(epoch2)
        print(epoch3)
    ...
    model_params3 = f'../../../../nvme/hashimoto.n/Output/model_params/{mag3}_train-{train_slide}_DArate-{DArate}_epoch-{epoch3}_opt.pth'
    ...
    DAMIL.load_state_dict(torch.load(model_params3,map_location='cpu'))
    feature_extractor_mag3 = DAMIL.feature_extractor
    model = MSDAMIL3(feature_extractor_mag1, feature_extractor_mag2, feature_extractor_mag3, class_predictor)
    model = model.to(rank)
    ...
    for epoch in range(EPOCHS):
        ...
        hori_train = HoriDataset.HoriDataset_multi3(
            train=True,
            transform=transform,
            pyvips=False,
        ...
        hori_valid = HoriDataset.HoriDataset_multi3(
            train=True,
            transform=transform,
            pyvips=False,
        ...
        if rank == 0:
            ...
            model_params = f'../../../../nvme/hashimoto.n/Output/model_params/{mag1}_{mag2}_{mag3}_train-{train_slide}_DArate-{DArate}_epoch-{epoch}_ms.pth'
