...
def eval(csv_file, output_file):
    ...
    for i in range (len(slide_id_list)):
        ...
        #correct_slide[i,3] = str(slide_prob[i][0])
        #correct_slide[i,4] = str(slide_prob[i][1])
        #orrect_slide[i,5] = str(slide_prob[i][2])
...
if __name__ == "__main__":
    ...
    DArate = 0
    #result_file = argvs[1]
    result_file = f'test_result/test_{mag1}_{mag2}_{mag3}_train-{train_slide}_DArate-{DArate}_ms.csv'
    output_file = f'test_predict/predict_{mag1}_{mag2}_{mag3}_train-{train_slide}_DArate-{DArate}_ms.csv'
